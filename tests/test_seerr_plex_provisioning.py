"""Invited Plex users get their Seerr account made for them.

Seerr does not import Plex users in the background - it creates them the
...
from app.services.companions.seerr import (
...
def _connection(url="http://seerr.local:5055"):
    ...
    conn.api_key = None
    conn.name = "Seerr"
    conn.connection_type = "seerr"
    return conn


def _resp(status=200, text=""):
    """Seerr replies carry a body we now read, so mocks must have one."""
...
def _seerr(xsrf="csrf-token-value"):
    ...
    with (
        patch(
            "app.services.companions.seerr.requests.Session",
            return_value=session,
        ),
        patch("app.services.companions.seerr.time.sleep") as sleep,
    ):
        yield session, sleep
...
def test_posts_the_plex_token_to_the_login_route():
    with _seerr() as (seerr, _):
        ...
        result = SeerrClient().provision_plex_user("plex-token-abc", _connection())
...
def test_sends_the_csrf_token_seerr_handed_us():
    """Without this every POST comes back 403 "invalid csrf token", whatever the
    payload. Seerr applies CSRF to API callers, not just browsers."""
    with _seerr(xsrf="the-expected-token") as (seerr, _):
        seerr.post.return_value = _resp(200)
        SeerrClient().provision_plex_user("t", _connection())
...
def test_no_api_key_is_sent():
    ...
    with _seerr() as (seerr, _):
        seerr.post.return_value = _resp(200)
        SeerrClient().provision_plex_user("plex-token-abc", _connection())
...
def test_trailing_slash_on_the_url_does_not_double_up():
    with _seerr() as (seerr, _):
        seerr.post.return_value = _resp(200)
        SeerrClient().provision_plex_user(
...
def test_retries_while_the_share_is_not_visible_yet():
    ...
    with _seerr() as (seerr, _):
        ...
        result = SeerrClient().provision_plex_user("t", _connection())
...
def test_does_not_retry_a_csrf_rejection():
    """A CSRF 403 is the request being refused, not the user. Seerr's CSRF
...
def test_skipped_when_no_url_is_configured():
    ...
    with _seerr() as (seerr, _):
        result = SeerrClient().provision_plex_user("t", _connection(url=None))
